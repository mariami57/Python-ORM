...
def product_quantity_ordered()->str:
    orders = ((((Product.objects.annotate(total_quantity=Sum('orderproduct__quantity')))
              .exclude(total_quantity=None))
              .values('name', 'total_quantity'))
              .order_by('-total_quantity'))

    return '\n'.join(f'Quantity ordered of {p["name"]}: {p["total_quantity"]}' for p in orders)


def ordered_products_per_customer()->str:
    ordered_products =(Order.objects.prefetch_related('orderproduct_set')
                       .order_by('id'))
    result = []
